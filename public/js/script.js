...
if(relogio)
{
    relogio.addEventListener('load', Relogio);
}
...
function validation(){
    var f = document.querySelector("btn-pesquisar");
    ...
    f.addEventListener("submit", function(e){
        ...
        for(var i = 0; i < this.elements.length; i++){
            ...
            if(item.type !== "submit"){
                if(item.value == ""){
                    item.focus();
                    item.style.backgroundColor="#EECCCC";
                    return false;
                }else{
...
function valid()
{
    ...
    for(var i = 0; i < itemsForm; i++)
    {
        if(formFinanceiro[i].type !== "submit")
        {
            ...
            {
                console.log(formFinanceiro[i].value)
...
if(btnPesquisarFinanceiro)
{
    btnPesquisarFinanceiro.addEventListener("click", function(e){
        ...
        sessionStorage.setItem('numPagina', numPagina.value)
        valor = sessionStorage.getItem('numPagina')
        numPag.value = valor
