<?php
    ...
    // require_once "../api/ConsultaFinanceiro.php";
    // require_once "../api/GerarBoleto.php";
    ...
    class Financeiro
    {
        ...
        {
            $financeiros = new Financeiros;
            $financeiros->alterStatusFinance($status, $id);
            header("Location: financeiro");
        }

        public function resulmoFinanceiro($data_inicio, $data_fim)
        {
            return $this->consultaFinanceiro($filtarPor, $data_inicio, $data_fim, $numPag);
        }
        ...
        public function gerarBoleto($id)
        {
            ...
            foreach($billetData as $data)
            {
                ...
                $modelCliente->setTipoPessoa(ModelCliente::PESSOA_FISICA);
                $mesangem->setLinha1($data['placa']);
                ...
                $boleto->setDataVencimento($trataDados->vencimento($data['vencimento']));
            }var_dump($boleto);die();
